        ...
        .then(function (response) {
            ...
            $(document).on('click', '#step_2', function () {
                if(venue == "") { // 극장 선택 안했으면
                    ...
                    if(content_type == "movie"){ // 영화관 선택했으면
                        $('#step2').removeClass("active"); // 극장선택 active 없애고
                        $('#movie_step1').addClass("active"); // 지역선택 active
                        currentStep += 1;
                    }
                    else{
                        nextStep();
            ...
            // 지역 선택
            let region = "";
            $(document).on('click', '.region_box', function () { // 극장 클릭
                if($(this).hasClass('cinema_box_active')){ // 이미 클릭됐으면
                    $(this).toggleClass('cinema_box_active'); // active 클래스 삭제
                    region = ""; // region 초기화
                }
                else{
                    $('.region_box').removeClass('cinema_box_active'); // active 클래스 초기화
                    $(this).toggleClass('cinema_box_active'); // this만 active 클래스 추가
                    region = $(this).text(); // 선택한 지역 저장
                }
            })

            // 지역선택 후 다음 버튼
            $(document).on('click', '#step_region', function (){
                if(region == ""){
                    alert("지역을 선택하세요");
                }
                else{
                    $('#movie_step1').removeClass('active'); // 지역선택 active 없애고
                    $('#movie_step2').addClass('active'); // n관 선택 active
                }
            })

            // 지역 선택 이전 버튼
            $(document).on('click', '#prev_step_region', function () {
                region = ""; // venue 초기화해서 다시 다음버튼 눌러서 들어오면 극장 선택
                $('.region_box').removeClass('cinema_box_active'); // 지역 선택 active 클래스 초기화
                $('#movie_step1').removeClass('active'); // 지역선택 active 없애고
                $('#step2').addClass('active'); // 극장선택 active
            })

            // n관 선택
            let hall = "";
            $(document).on('click', '.hall_box', function () { // 극장 클릭
                if($(this).hasClass('cinema_box_active')){ // 이미 클릭됐으면
                    $(this).toggleClass('cinema_box_active'); // active 클래스 삭제
                    hall = ""; // hall 초기화
                }
                else{
                    $('.hall_box').removeClass('cinema_box_active'); // active 클래스 초기화
                    $(this).toggleClass('cinema_box_active'); // this만 active 클래스 추가
                    hall = $(this).text(); // 선택한 n관 저장
                }
            })

            // n관 선택 후 다음 버튼
            $(document).on('click', '#step_hall', function (){
                if(hall == ""){
                    alert("영화관을 선택하세요");
                }
                else{
                    $('#movie_step2').removeClass('active'); // n관 선택 active 없애고
                    $('#step3').addClass('active'); // 시간 선택 active
                }
            })

            // n관 선택 이전 버튼
            $(document).on('click', '#prev_step_hall', function () {
                hall = ""; // venue 초기화해서 다시 다음버튼 눌러서 들어오면 극장 선택
                $('.hall_box').removeClass('cinema_box_active'); // b관 선택 active 클래스 초기화
                $('#movie_step2').removeClass('active'); // n관 선택 active 없애고
                $('#movie_step1').addClass('active'); // 지역선택 active
            })
            ...
            $(document).on('click', '#prev_step_3', function () {
                ...
                if(hall != ""){ // 영화관 선택에서 넘어온거면
                    $('#step3').removeClass('active');
                    $('#movie_step2').addClass('active');
                    currentStep -= 1;
                }
                else{
                    prevStep();
            ...
            $(document).on('click', '#tck_complete', function () {
                ...
                localStorage.setItem("s_label", s_label);
                localStorage.setItem('toUrl', window.location.href); // 지금 Url을 localStorage에 저장
                if(user){ // 로그인 되어있으면
                    if(s_label == ""){
                        alert("좌석을 선택하세요");
                    }
