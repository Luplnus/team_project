document.addEventListener("DOMContentLoaded", function () {
    axios.get('http://54.180.234.163:5002/api/theaters').then(function (response) {
        ...
        data.forEach(function(item){
            ...
            slide.innerHTML =
                `<img src="http://54.180.234.163:5002/image-proxy?url=${encodeURIComponent(item.t_poster)}" alt="${item.t_title}" onerror="this.src='https://via.placeholder.com/240x204?text=No+Image'">`

            slide.addEventListener('click', function () {
                const proxyUrl = `http://54.180.234.163:5002/image-proxy?url=${encodeURIComponent(item.t_poster)}`
                document.querySelector('.post_img').src = proxyUrl;
