...
<div class="ticket_container active" id="step1">
    <div class="ticket_box">
        ...
        <div class="tck_btn_box">
            <button class="tck_btn tck_btn_prev">이전</button>
...
<script>
    ...
    $('.content_name').text('${musicalDto.mu_title}');
    $('.content_genre').text('${musicalDto.mu_genre}');
    $('.content_runtime').text('${movieDto.m_show_tm}');
        ...
        if($(this).attr('id') == 'tag_box_theater'){
            ...
            $('#movieList').html( // 영화 리스트
                `<c:forEach var="movie_list" items="${movieList}">
                    <div class="mv_nm" data-poster="${movie_list.m_poster_url}" data-content_id="${movie_list.m_movie_cd}" data-type="movie">
    ...
    $(document).on('click', '#step_1', function () {
        if(content_id == "") { // 작품 선택 안했으면
            ...
            <c:forEach var="s_is_able" items="${s_is_available}">
            if(content_id == '${s_is_able.mu_id}'){ // s_is_available이 false인 s_label이 있다면
                tmp[count++] = '${s_is_able.s_label}'; // 배열에 s_label 저장
            }
            </c:forEach>
            $('.tck_seat_box button').each(function(){ // 좌석 버튼 각각 불러와서
                const seatLabel = $(this).attr('id'); // D09
                if(tmp.includes(seatLabel)){ // tmp에 해당 좌석 번호가 있으면
    ...
    // 이전 버튼
    $(document).on('click', '.tck_btn_prev', function () {
        prevStep();
    })
    ...
    const dayOfWeekIndex = today.getDay(); // 0 (일) ~ 6 (토)
    ...
    $(document).on('click', '.tck_seat_box button', function (){
        if($(this).hasClass('selected')){
            ...
            $('.tck_seat_box button').removeClass('selected');
            $(this).toggleClass('selected');
            s_label =  $(this).text();
        }
    })
